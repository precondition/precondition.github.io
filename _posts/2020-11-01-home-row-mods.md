...
[^8]: Before you go ahead and do that, do keep in mind that Shift is sometimes used by itself in applications. In many video games, Shift is used to crouch (though, that would be the least of your problems if you want to play video games with Auto Shift on...) and JetBrains IDEs have you double-tap either Shifts by themselves to open up the [Search Everywhere](https://blog.jetbrains.com/idea/2020/05/when-the-shift-hits-the-fan-search-everywhere/) window. Make sure to also read the documentation section on [modifiers and Auto Shift](https://docs.qmk.fm/#/feature_auto_shift?id=modifiers) first.
